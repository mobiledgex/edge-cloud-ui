...
class SiteFour extends React.Component {
    constructor(props) {
        ...
        this.state = {
            ...
            userToken: null,
            userName: "",
            regions: [{ key: 1, text: "All", value: "All" }],
    ...
    getAdminInfo = async token => {
        ...
        _self.setState({
            currentVersion: process.env.REACT_APP_BUILD_VERSION
    ...
    enalbeSteps = () => {
        ...
        if (
            _self.props.params.mainPath === "/site4" &&
            _self.props.params.subPath === "pg=newOrg"
        ) {
        ...
        let element = elmentName
            ? document.getElementsByClassName(
                  elmentName[0].element.replace(".", "")
              )
            : [];
        if (enable) {
    ...
    UNSAFE_componentWillMount() {
        ...
        _self.setState({
            activeItem: localStorage.selectMenu
                ? localStorage.selectMenu
                : "Organizations",
            headerTitle: localStorage.selectMenu
                ? localStorage.selectMenu
                : "Organizations"
        });
    ...
    componentWillReceiveProps(nextProps, nextContext) {
        try {
            ...
            _self.setState({
                contHeight:
                    (nextProps.size.height - _self.headerH) / 2 - _self.hgap
            });
            _self.setState({
                contWidth: window.innerWidth - _self.state.menuW
            });
            _self.setState({ userToken: nextProps.userToken });
            _self.setState({
                userName:
                    nextProps.userInfo && nextProps.userInfo.info
                        ? nextProps.userInfo.info.Name
                        : null
            });
        } catch (e) {}
        ...
        if (
            nextProps.alertInfo !== _self.props.alertInfo &&
            nextProps.alertInfo.mode
        ) {
            Alert.closeAll();
        ...
        if (formKey.length) {
            if (nextProps.formInfo[formKey[0]]["submitSucceeded"]) {
                if (
                    nextProps.formInfo[formKey[0]]["submitSucceeded"] === true
                ) {
        ...
        if (nextProps.regionInfo.region.length && !_self.state.regionToggle) {
            ...
            let newRegions = Object.assign([], _self.state.regions).concat(
                getRegions
            );
    ...
    reduceAuditCount(all, data) {
        ...
        all.map((item, i) => {
            if (savedArray && JSON.parse(savedArray).length) {
                //이전에 없던 데이터 이면 추가하기
                if (
                    JSON.parse(savedArray).findIndex(k => k == item.traceid) ===
                    -1
                )
                    addArray.push(item.traceid);
            } else {
    ...
    render() {
        ...
        return (
            <div className="view_body" style={{ height: "100%" }}>
                ...
                {hints ? <Hints enabled={hintsEnabled} hints={hints} /> : null}
                ...
                <SideNav
                    onOptionClick={_self.handleItemClick}
                    email={_self.state.email}
                    data={_self.props.userInfo.info}
                    helpClick={_self.enalbeSteps}
                    gotoUrl={_self.gotoUrl}
                />
                <Motion
                    defaultStyle={defaultMotion}
                    style={_self.state.setMotion}
                >
                    {interpolatingStyle => (
                        <div
...
const mapStateToProps = state => {
    ...
    return {
        ...
        changeRegion: state.changeRegion.region
            ? state.changeRegion.region
            : null,
...
export default withRouter(
    connect(
        mapStateToProps,
        mapDispatchProps
    )(sizeMe({ monitorHeight: true })(SiteFour))
);

...
const panes = [
    {menuItem: 'Details', render: (props) => <Tab.Pane>{detailViewer(props, 'detailViewer')}</Tab.Pane>},
    {
        menuItem: 'Monitoring', render: (props) => {

            console.dir('MonitoringMonitoring====>' + JSON.stringify(props));

            console.dir('slkfsdkf====sldkflsdkflksdf');
            console.dir('slkfsdkf====sldkflsdkflksdf');
            console.dir('slkfsdkf====sldkflsdkflksdf');
            console.dir('slkfsdkf====sldkflsdkflksdf');
            return (
                <Tab.Pane><MonitoringViewer data={props}/></Tab.Pane>
            )
        }
    }
]
    ...
    <Fragment>
        {(type === 'detailViewer') ?
            <Table celled collapsing
                   style={{width: '100%', height: '100%', border: 'none', display: 'flex', flexDirection: 'column'}}>
                <Table.Header>
                    <Table.Row>
                        <Table.HeaderCell width={6}>
                            <div style={{display: 'flex', justifyContent: 'center'}}>Subject</div>
                        </Table.HeaderCell>
...
class PageDetailViewer extends React.Component {
                             ...
                             panes={
                                 (this.state.userRole === 'AdminManager' && page === 'appInst') ? panesCommand
                                     ...
                                     : (page === 'appInst') ? panesCommand
                                         : panes}{...panelParams}
                             gotoUrl={this.gotoUrl} toggleSubmit={this.state.toggleSubmit}
                             error={this.state.validateError} onTabChange={this.onChangeTab}/>
                    </div>
                </div>
                :
                <div className="round_panel" key={i}>
    ...
    getParams = (page, data, store) => (
        (page === 'appInst' && _self.resources[page].length) ?
            _self.resources[page].map((valid) => {


                let results=this.makeFormApp(data, valid, store.userToken)

                console.log('makeFormApp====>' + results);
    ...
    makeList = (values, label, i) => (
        <Grid.Row columns={2} key={i}>
            ...
            <Grid.Column width={11}>
                <div
                    style={{wordWrap: 'break-word'}}>{(typeof values[label] === 'object') ? JSON.stringify(values[label]) : String(values[label])}</div>
            </Grid.Column>

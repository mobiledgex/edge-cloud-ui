...
let _self = null;
export default hot(
    class ListDetailViewer extends React.Component {
        constructor() {
            super();
            this.state = {
                dummyData: [],
        ...
        componentWillReceiveProps(nextProps, nextContext) {
            ...
            if (nextProps.data) {
                ...
                let listOfDetailJsxResults = [];

                regKeys.map((key, index) => {

                    //@desc: index가 0일때 해더를 붙인다..
                    //@desc: Attach a header when index is zero.
                    if (index === 0) {
                        listOfDetailJsxResults.push(
                            <Grid.Row columns={3} style={{backgroundColor: '#292B33', margin: 0}}>
                                <Grid.Column style={{}}>
                                    <View style={{
                                        //backgroundColor: '',
                                        alignItems: 'flex-start',
                                        justifyContent: 'flex-start',
                                        alignSelf: 'center', height: 18,
                                        fontSize: 15,
                                        color: 'white',
                                    }}>
                                        Key
                    ...
                    if (key !== 'Edit') {
                        listOfDetailJsxResults.push(
                            <Grid.Row columns={2} key={index} style={{backgroundColor: '#22242C'}}>
                ...
                this.setState({
                    listOfDetail: listOfDetailJsxResults,

...
class MapWithListView extends React.Component {
    ...
    detailView(item) {
        ...
        {
            ((localStorage.selectRole === 'OperatorManager' || localStorage.selectRole === 'OperatorContributor' || localStorage.selectRole === 'OperatorViewer') && item.Operator !== localStorage.selectOrg) ?
                _self.setState({viewMode: 'listView'}) : _self.setState({viewMode: 'detailView'})
        }
        {
            ((localStorage.selectRole === 'OperatorManager' || localStorage.selectRole === 'OperatorContributor' || localStorage.selectRole === 'OperatorViewer') && item.Operator !== localStorage.selectOrg) ?
                _self.props.handleDetail({data:item, viewMode:'listView'}) : _self.props.handleDetail({data:item, viewMode:'detailView'})
        }
    }
    jsonView = (jsonObj) => (

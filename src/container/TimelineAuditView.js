            ...
            class TimelineAuditView extends React.Component {
                ...
                componentWillReceiveProps = async (nextProps, nextContext) => {
                    ...
                    if (nextProps.data !== this.props.data) {
                        ...
                        let storageSelectedTraceidList = JSON.parse(
                            localStorage.getItem("selectedTraceid")
                        );
                        ...
                        if (nextProps.data.data && nextProps.data.data.length) {
                            ...
                            this.setState({
                                ...
                                currentTraceid:
                                    dummyConts[this.state.selectedIndex][
                                        "traceid"
                                    ]
                            });
                            if (dummyConts[this.state.selectedIndex])
                                this.setAllView(
                                    dummyConts[this.state.selectedIndex],
                            ...
                            for (let i in auditList) {
                                ...
                                let nameIndex = nameOptions.findIndex(
                                    t => t.value === makeOperName
                                );
                                if (nameIndex === -1) {
                                    nameOptions.push({
                                        key: makeOperName,
                                ...
                                let renderValue = this.typeRender(
                                    this.makeOper(operName)
                                );
                                let typesIndex = typeOptions.findIndex(
                                    t => t.value === renderValue
                                );
                            ...
                            for (let i in auditList) {
                                ...
                                if (auditList[i].status === 200) {
                                    ...
                                    let storageSelectedTraceidIndex = storageSelectedTraceidList
                                        ? storageSelectedTraceidList.findIndex(
                                              s => s === traceid
                                          )
                                        : -1;
                            ...
                            if (
                                !check ||
                                storageSelectedTraceidList.length > 200
                            ) {
                ...
                makeNotUTC = time => {
                    let newTime = moment(time).unix();
                    let makeTime = moment(newTime)
                        .utc()
                        .format("HH:mm:ss:SSS");
                    if (makeTime === _self.sameTime) {
                        _self.addCount++;
                        makeTime = moment(newTime + 1000)
                            .utc()
                            .format("HH:mm:ss");
                ...
                setStorageData(data, type) {
                    ...
                    let storageTraceidList = JSON.parse(
                        localStorage.getItem(localStorageName)
                    );

                    if (storageTraceidList) {
                        traceidList = storageTraceidList;
                        let storageTraceidIndex = storageTraceidList.findIndex(
                            s => s === data
                        );
                ...
                onItemSelect = (item, i) => {
                    ...
                    times.map((time, index) => {
                        if (Date.parse(this.getParseDate(time)) === item) {
                            ...
                            this.onHandleIndexClick({
                                value: i,
                                traceid: status[index].traceid
                            });
                            if (status[index].status !== 200) {
                                if (storageSelectedTraceidList) {
                                    let storageSelectedTraceidIndex = storageSelectedTraceidList
                                        ? storageSelectedTraceidList.findIndex(
                                              s => s === status[index].traceid
                                          )
                                        : -1;
                                    if (storageSelectedTraceidIndex === -1) {
                                        this.setState({
                                            unCheckedErrorCount:
                                                this.state.unCheckedErrorCount -
                                                1
                                        });
                                    }
                                }
                ...
                setRequestView(dummyConts, sId) {
                    if (dummyConts && dummyConts["request"]) {
                        if (dummyConts["request"].indexOf("{") > -1) {
                            let dataLenght = dummyConts["request"].split(
                                '{"data":'
                            ).length;
                            if (dataLenght > 1) {
                                this.setState({
                                    requestData: {
                                        data: dummyConts["request"].split(
                                            '{"data":'
                                        )
                                    }
                                });
                            ...
                            this.setState({
                                requestData: { request: dummyConts["request"] }
                            });
                        }
                    } else {
                ...
                setResponseView(dummyConts, sId) {
                    if (dummyConts.operationname.includes("/ws/")) {
                        dummyConts.response = this.convertWSResponsetoJSON(
                            dummyConts.response
                        );
                    }
                    if (
                        dummyConts &&
                        dummyConts["response"].indexOf("{") > -1
                    ) {
                        let dataLenght = dummyConts["response"].split(
                            '{"data":'
                        ).length;
                        if (dataLenght > 1) {
                            this.setState({
                                responseData: {
                                    data: dummyConts["response"].split(
                                        '{"data":'
                                    )
                                }
                            });
                ...
                onClickUnCheckedError = e => {
                    let unCheckedToggle =
                        e === "current"
                            ? false
                            : this.state.unCheckedErrorToggle;
                    let value = "uncheck";
                    let allData = this.state.rawAllData;
                    let timelineList = [];
                    let tasksList = [];
                ...
                getWidth = () => {
                    return localStorage.getItem("navigation") == 0
                ...
                render() {
                    return (
                        ...
                        >
                            <Toolbar>
                                <label className="content_title_label">
                                    Audit Logs
                                </label>
                                <div
                                    style={{
                                    ...
                                    <div className="page_audit_history_option">
                                        ...
                                        <div className="page_audit_history_option_period">
                                            <button className="page_audit_error_box with_button">
                                                ...
                                                <div className="page_audit_badge_number">
                                                    {
                                                        this.state
                                                            .unCheckedErrorCount
                                                    }
                                            ...
                                            <button
                                                className="page_audit_error_button"
                                                onClick={
                                        ...
                                        <div className="page_audit_history_option_period">
                                            <div
                                                className="page_audit_error_box with_button"
                                                onClick={this.onCurrentClick}
                                            >
                                                <div className="page_audit_error_label">
                                                    (UTC){" "}
                                                    {moment(this.state.realtime)
                                                        .utc()
                                                        .format(
                                                            "YYYY-MM-DDTHH:mm"
                                                        )}
                            ...
                            <div className="mexListView">
                                ...
                                >
                                    {this.state.timesList.length > 0 ? (
                                        <CalendarTimeline
                                            ...
                                            onCanvasClickCloseMap={
                                                this.onClickCavasCloseMap
                                            }
                                ...
                                >
                                    <span style={{ color: "#c8c9cb" }}>
                                        {this.state.closeMap ? "Show" : "Hide"}
                                    </span>
                                    <Icon
                                        name={
                                            this.state.closeMap

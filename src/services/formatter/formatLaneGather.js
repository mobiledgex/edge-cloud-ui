...
OCCURRED_DATE : "20180327190109"
TIMEMARK : "20180327190109"
CONFIRMYN : "N"
AL_CNT : 0
CONFIRMDATE : null
PLID : "097"
...
let generateData = (datas) => {
    ...
    let rows = [];
    datas.map((data) => {
        rows.push({"title":"상태", "nLevel":data['NOTICE_LEVEL'],"value":[data['CONFIRMYN'], retunDate(data['OCCURRED_DATE']),data['LANENO'],data['KIND'],data['ERRLIST']]})
    })
    if(datas.length === 0) rows.push({"title":"상태","value":["","","","",""]})
    let laneGather = {"title":"차로 기기 이상내역", "value": {
...
const FormatLaneGather = (props) => (
    generateData(props)
)

export default FormatLaneGather;

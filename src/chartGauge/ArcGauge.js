...
class ArcGauge extends Component {
    ...
    constructor() {
        ...
        this.maxTemper = 180;
        this.roundBoards = [
            "/assets/gauge/chart_gauge_in_circle_rainbow.png",
            "/assets/gauge_bk_yellow.png",
            "/assets/gauge_bk_orange.png",
            "/assets/gauge_bk_red.png"
        ];
    ...
    componentDidMount() {
        ...
        let interval = () => {
            ...
            self.setState({
                currentTemp: currentTemper,
                degree: self.makeDegree(currentDegrees, self),
    ...
    renderGauge(value, limits, type, title) {
        ...
        if (type === "temp") {
            statusBoard =
                currentTemper < limits.minor1 && currentTemper > limits.major1
                    ? this.roundBoards[1]
                    : currentTemper >= limits.major1 &&
                      currentTemper < limits.critical1
                    ? this.roundBoards[2]
                    : currentTemper >= limits.critical1
                    ? this.roundBoards[3]
                    : this.roundBoards[0];
        ...
        if (type === "humi") {
            if (
                currentTemper > limits.minor1 &&
                currentTemper < limits.minor2
            ) {
                //정상
                ...
                if (
                    ...
                    statusBoard = this.roundBoards[1];
                //minor(max)
                if (
                    currentTemper >= limits.minor2 &&
                    currentTemper < limits.major2
                )
                    statusBoard = this.roundBoards[1];
                //major(min)
                if (
                    ...
                    statusBoard = this.roundBoards[2];
                //major(max)
                if (
                    currentTemper >= limits.major2 &&
                    currentTemper < limits.critical2
                )
                    statusBoard = this.roundBoards[2];
                //critical(min/max)
                if (
                    ...
                    statusBoard = this.roundBoards[3];
            }
        }
		...
		*/
        ...
        console.log(
            "20191030 percent value == ",
            percentVal,
            " : degree == ",
            self.makeDegree(value, self)
        );

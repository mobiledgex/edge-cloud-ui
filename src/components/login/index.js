...
const FormContainer = props => (
    <Grid className="signUpBD">
        ...
        <div className="loginValidation">{props.login_danger}</div>
        <Grid.Row>
            <Grid.Column>
                <Button
                    onFocus={() => props.self.onFocusHandle(true)}
                    onBlur={() => props.self.onFocusHandle(false)}
                    onClick={() => props.self.onSubmit()}
                >
                    Log In
                </Button>
            </Grid.Column>
        ...
        <Grid.Row>
            <div
                ...
                onClick={() => props.self.handleClickLogin("forgot")}
            >
                Forgot Password?
            </div>
        </Grid.Row>
    ...
    <Grid className="signUpBD">
        ...
        <Grid.Row>
            <Grid.Column>
                <Button
                    onFocus={() => props.self.onFocusHandle(true)}
                    onfocusout={() => props.self.onFocusHandle(false)}
                    onClick={() => props.self.onSendEmail()}
                >
        ...
        <Grid.Row>
            <Grid.Column>
                <Button onClick={() => props.self.onSendEmail("back")}>
                    Return to Log In
                </Button>
            </Grid.Column>
        </Grid.Row>
...
const ForgotMessage = props => (
    <Grid className="signUpBD">
        ...
        <Grid.Row>
            <Grid.Column>
                <Button
                    onFocus={() => props.self && props.self.onFocusHandle(true)}
                    onfocusout={() =>
                        props.self && props.self.onFocusHandle(false)
                    }
                    onClick={() => props.self.returnSignin()}
                >
                    Return to Sign In
                </Button>
            </Grid.Column>
...
const ResetPassword = props => (
    <Grid className="signUpBD">
        ...
        <RegistryResetForm
            onSubmit={() => console.log("ProfileForm was submitted")}
        />
        <Grid.Row>
            <span>
...
const FormResendVerify = props => (
    <Grid className="signUpBD">
        ...
        <Grid.Row>
            <span className="login-text">
                Enter your email address and we will send you a link to verify
                your email.
            </span>
        </Grid.Row>
        <Grid.Row>
        ...
        <Grid.Row>
            <Grid.Column>
                <Button
                    onFocus={() => props.self.onFocusHandle(true)}
                    onfocusout={() => props.self.onFocusHandle(false)}
                    onClick={() => props.self.onSendEmail("verify")}
                >
                    Send verify email
                </Button>
            </Grid.Column>
...
const FormSignUpContainer = props => (
    <Grid className="signUpBD">
        ...
        <RegistryUserForm
            onSubmit={(a, b) =>
                console.log("20190906 ProfileForm was submitted", a, b)
            }
            userInfo={{
...
const SuccessMsg = props => (
    <Grid className="signUpBD">
        {!(props.msg.indexOf("created") > -1) && (
            <Grid.Row>
                <span className="title" onClick={() => console.log(props.msg)}>
                    {String(props.msg)}
                </span>
            </Grid.Row>
        )}
                ...
                <Grid.Row>
                    <Grid.Column>
                        <Button
                            ...
                            onfocusout={() => props.self.onFocusHandle(false)}
                            onClick={() =>
                                props.self.handleClickLogin("signup")
                            }
                        >
                            <span>Sign Up</span>
                        </Button>
        ...
        <Grid.Row>
            <Grid.Column>
                <Button
                    onFocus={() => props.self.onFocusHandle(true)}
                    onfocusout={() => props.self.onFocusHandle(false)}
                    onClick={() => props.self.handleClickLogin("login")}
                >
                    <span>Log In</span>
                </Button>
            </Grid.Column>
...
class Login extends Component {
    ...
    componentDidMount() {
        ...
        if (oldUserInfo) {
            if (
                oldUserInfo.date &&
                moment().diff(oldUserInfo.date, "minute") >= 60
            ) {
    ...
    componentWillReceiveProps(nextProps) {
        let store = localStorage.PROJECT_INIT
            ? JSON.parse(localStorage.PROJECT_INIT)
            : null;
        if (nextProps.values) {
            if (nextProps.submitSucceeded) {
                ...
                if (nextProps.loginMode === "resetPass") {
                    serviceMC.sendRequest(
                        self,
                        {
                            method: serviceMC.getEP().RESET_PASSWORD,
                            data: {
        ...
        if (nextProps.loginMode === "login") {
            ...
            nextProps.loginMode === "signuped" &&
            nextProps.createSuccess
        ) {
            localStorage.setItem("userInfo", null);
        ...
        if (mcRequest) {
            if (mcRequest.response) {
                ...
                if (response.data.token) {
                    ...
                    if (Alert) {
                        self.props.handleAlertInfo(
                            "error",
                            response.data.message
                        );
                        //goto reqeuset verify email ....
                        if (
                            response.data.message.indexOf("not verified") > -1
                        ) {
    ...
    onSendEmail(mode) {
        if (mode === "verify") {
            ...
            serviceMC.sendRequest(
                self,
                {
                    method: serviceMC.getEP().RESET_PASSWORD_REQUEST,
                    data: data
                },
                this.receiveForgoten,
                this
            );
        }
    }
    ...
    render() {
        return this.state.session !== "open" ? (
            <Container>
...
const mapStateToProps = state => {
    ...
    let createSuccess = state.createAccount
        ? state.createAccount.created.success
        : null;
    let userInfo = state.createAccount
        ? state.createAccount.created.info
        : null;
    return {
...
const mapDispatchProps = dispatch => {
    return {
        ...
        mapDispatchToLoginWithPassword: data =>
            dispatch(actions.loginWithEmailRedux({ params: data })),
